...
from database.delete_document import clear_firmware_database
from firmware_handler.firmware_delete import delete_firmware_by_id
from firmware_handler.firmware_version_detect import detect_by_build_prop
from firmware_handler.firmware_os_detect import set_firmware_by_filenames
...
class FirmwareMassImport(Resource):
    ...
    def post(self, create_fuzzy_hashes):
        ...
        job = None
        try:
            job = Job.fetch("firmware_import", connection=app.redis)
            status = job.get_status(refresh=True)
        except NoSuchJobError:
            pass

        if status is None or (status != "queued" or status != "started" or status != "deferred" or status != "scheduled"):
            job = app.rq_task_queue_high.enqueue(start_firmware_mass_import,
                                                 create_fuzzy_hashes,
                                                 job_timeout=60 * 60 * 24 * 7,
                                                 job_id="firmware_mass_import")
        return {"id": job.get_id()} if job is not None else 400
...
class GetLatestFirmware(Resource):
    ...
    def get(self):
        ...
        """
        response = "", 400
        # TODO add caching to increase performance and prevent DoS
        try:
...
class SetFirmwareOsVendor(Resource):
    ...
    def post(self, os_vendor):
        ...
        :param os_vendor: str - OS Vendor name to add to the firmware.
